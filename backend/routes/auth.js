...
router.post('/send-otp', sendOtpLimiter,sendOtp);  // ❌ No rate limiting, no input validation

// POST /api/auth/verify-otp - Verify OTP and register/login user
router.post('/verify-otp', verifyOtpLimiter, verifyOtpAndRegister);  // ❌ No rate limiting, no input validation
